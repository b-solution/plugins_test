...
class LeadsController < ApplicationController
  ...
  def search_existance
    respond_to do |format|
      format.js{
        @type  = params[:type]
        @value = params[:value].split(',')
        if @type.include?('phone')
          @exist = Contact.where(phone: @value)
        elsif @type.include?('skype')
          @exist = Contact.where(skype_name: @value)
        elsif @type.include?('email')
          @exist = Contact.where(email: @value)
        end
      }
    end
  end
  ...
  def context_menu
    ...
    @can = {:edit => (@contact && @contact.editable?) || (@contacts && @contacts.collect{|c| c.editable?}.inject{|memo,d| memo && d}),
            ...
            :create => (@project && User.current.allowed_to?(:add_leads, @project)),
  ...
  def bulk_update
    ...
    redirect_back_or_default({:controller => 'leads', :action => 'index', :project_id => @project})
  end
  ...
  def redirect_on_create(options)
    if options[:redirect_on_success].to_s.match('^(http|https):\/\/')
      ...
      render :action => 'show', :status => :created, :location => lead_url(@contact)
    end
  end
